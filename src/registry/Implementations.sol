...
contract Implementations is Setup {
    ...
    IndexInterface public immutable polyIndex;

    constructor(address _polyIndex) {
        polyIndex = IndexInterface(_polyIndex);
    }

    modifier isMaster() {
        require(msg.sender == polyIndex.master(), "Implementations: not-master");
        _;
    }
    ...
    function setDefaultImplementation(address _defaultImplementation) external isMaster {
        ...
        require(
            _defaultImplementation != defaultImplementation, "Implementations: _defaultImplementation cannot be same"
        );
...
contract PolyImplementations is Implementations {
    constructor(address _polyIndex) Implementations(_polyIndex) {}
